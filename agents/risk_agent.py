"""
RISK Agent — Senior Risk Analyst.

Systematically identifies, categorizes, and assesses all material risks
facing a startup across market, technical, regulatory, financial,
operational, and team dimensions.
"""
...
from schemas.risk import RiskAgentOutput
from services.claude_client import ClaudeClient, make_phase2_client


class RiskAgent(BaseAgent):
    """
    Senior Risk Analyst that produces a structured risk register with
    probability/impact assessments and mitigation strategies.
    """

    agent_name = "risk"
    prompt_filename = "risk_system_prompt.txt"
    ...
    def _build_prompt_context(
        ...
        lines = [
            "STARTUP SUBMISSION — RISK ANALYSIS",
            "=" * 40,
        ...
        if upstream_context:
            ...
            lines += [
                ...
                f"EVAL Scores — Market: {phase1.get('market_viability_score', 'N/A')}, "
                f"Feasibility: {phase1.get('feasibility_score', 'N/A')}, "
                f"Scalability: {phase1.get('scalability_score', 'N/A')}",
                f"Team Fit Score: {phase1.get('founder_market_fit_score', 'N/A')}",
            ]
            red_flags = phase1.get("red_flags", [])
            if red_flags:
                lines += ["Red Flags from EVAL Agent (must be included in risk register):"]
                for flag in red_flags:
                    lines.append(f"  - {flag}")
            team_risks = phase1.get("team_risk_factors", [])
            if team_risks:
                lines += ["Team Risk Factors from TEAM Agent:"]
        ...
        lines += [
            "",
            "=" * 40,
            "Produce a comprehensive risk register. Output ONLY valid JSON.",
        ]
    ...
    def _parse_json_response(self, raw: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            validated = RiskAgentOutput(**raw)
            return validated.model_dump()
        except ValidationError as exc:
            raise ValueError(
                f"RISK agent output failed schema validation: {exc}"
            ) from exc
